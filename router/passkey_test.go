...
func setupPasskeyTestRouter(t *testing.T, enabled bool) (*gin.Engine, *gin.Engine, *config.Config) {
	t.Helper()
	...
	return public, protected, cfg
}

func TestPasskeyRegisterBeginBadJSON(t *testing.T) {
	r, _, _ := setupPasskeyTestRouter(t, false)
...
func TestPasskeyListCredentials(t *testing.T) {
	_, protected, _ := setupPasskeyTestRouter(t, false)
	w := httptest.NewRecorder()
