...
func (s *Service) CreateMetric(metric *domain.Metric) error {
	return s.metricRepo.CreateMetric(metric)
}

func (s *Service) GetMetricByID(id int32) (*domain.Metric, error) {
	return s.metricRepo.GetMetricByID(id)
}

func (s *Service) GetMetricByName(name string) (*domain.Metric, error) {
...
func (s *Service) UpdateMetric(metric *domain.Metric) error {
...
func (s *Service) ListMetricRecords(metricID int32) ([]domain.MetricRecord, error) {
	return s.metricRepo.ListMetricRecordsByMetricID(metricID)
}

func (s *Service) RecordMetric(input RecordMetricInput) (*domain.Metric, error) {
...
func (s *Service) IncrementMetric(input IncrementMetricInput) (*domain.Metric, error) {
	metric, err := s.metricRepo.GetMetricByName(input.MetricName)
	if err != nil {
		return nil, fmt.Errorf("metric not found: %w", err)
	}
